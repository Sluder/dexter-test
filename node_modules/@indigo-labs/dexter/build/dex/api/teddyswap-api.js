    ...
    liquidityPools(assetA, assetB) {
        return this.api.get('/front/pools').then((response) => {
            return response.data.map((poolResponse) => {
                ...
                let liquidityPool = new LiquidityPool(TeddySwap.identifier, tokenA, tokenB, BigInt(poolResponse.lockedX.amount), BigInt(poolResponse.lockedY.amount), '', // Not supplied
